/*
  비동기 방식의 코드문제로
  return flase 또는 return true가 실행되지 않는 문제를 해결하기 위하여
  promise(약속, 보장) 방식으로 코드를 변경한다
  함수 선언문제 async 키워드를 부착
...
const st_num_fetch = (st_num) => {
  ...
  // 이 함수가 ture나 false를 return 할 것으로 기대하고
  // 작성을 했으나
};
...
const save_cb = async () => {
  ...
  // JS 코드를 사용하여 Client Sixe Validation을 수행
  // JS 코드를 사용하여 Front 단에서 유효성 검사
  // 값을 입력하지 않았을 때 경고 메시지를 보여주고 유효성 검사 중단
  if (st_num.value === "") {
  ...
  /*
    ...
    문자열형 숫자를 숫자로 바꾸려고 할 때 숫자형이 아닌 다른 데이터가 포함되어 숫자로 바꿀 수 없을 경우
    Java에서는 NumberFormatException이 발생하고 코드가 중단된다
    코드 중단을 방지하기 위하여 try-Catch로 처리를 해주어야 한다
  
    그런데 JS에서는 그러한 상황에 Exception이 발생하는 대신
    NaN(Not a Number)이라는 값을 return한다
  
    JS에서 사용되는 특별한 데이터들이 있다
    NaN(숫자가 아니다)
    undefineded(정의되지 않은 변수)
  ...
  // 아래의 if() 명령문은 num_grade에 담긴 값이 'NaN 이면' 이라고 물어보는 명령이다
  if (!num_grade) {
  ...
  // 이제 모든 유효성 검사를 정상적으로 통과했으므로 서버로 데이터 전송
