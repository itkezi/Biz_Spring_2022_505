...
public class HomeController {
	...
	/*
	 * @Valid : vo(dto) 클래스에 유효성 검사 코드를 추가해 두고 form으로부터 데이터가 전송되어 오면 자체적으로 검사를 수행한다
	 * 
	 * 만약 VO에 설정된 유효성 검사가 한 개라도 통과하지 못하면 BindingResult 객체에 오류 메시지가 포함된다
	 * BindingResult 객에의 hasErroers() method를 사용하여 오류가 있는지 확인하고 다시 원래 form(*.jsp)로
	 * 되돌려 보낸다
	 * 
	 * @valid와 BindingResult는 순서를 바꾸면 안된다
	 * 
	 * 보통 파일 업로드를 받을 때는 파일 정보를 수신할 매개변수에 @RequestParam()을 사용한다
	 * 하지만 여러 파일을 수신하는 MultipartHttpServletRequest를 매개변수로 사용할 때는 절대 @RequestParam()을 사용하면 안된다
	 */
	...
	/*
	 * @RequestParam()
	 * 1. client에서 보내는 변수와 실제 사용하는 변수의 이름이 다를 경우
	 * 2. 매개변수가 숫자형 일 때 client에서 변수에 값을 담지 않고 요청을 하면 내부 오류가 발생한다 
	 * 		이때, required를 false로 선언하고 defaultValue를 설정하면 오류를 일부 방지할 수 있다
	 */
	@ResponseBody
