 ...
 * 
 * 그래서 Service, Dao interface를 구현할 때 공통으로 사용할 parent interface를 생성하고
 * 기본 CRUD method는 상속받아서 사용하도록 구현한다
 * 
 * Service, Dao interface에는 VO 클래스와, Primary Key 칼럼의 데이터 타입이 모두 다르게 포함된다
 * 
 * 상속받은 interface에서 자신의 VO 클래스와 Primary Key 칼럼의 데이터 타입을 지정할 수 있도록
 * Generic Type의 interface로 구현해놓는다
 * 
 * 여기에서 선언하는 Generic type이란 아직 구현되지 않은 임의의 type이며
 * 이 interface를 상속받아 구현할 때 비로소 구체화 된다
 */
