...
public class SelfitController {
	...
	// localhost:9090/selfit/workout/dayset/01-001 URL로 요청을 받고
	// sc_num에 등급 ID(01-001) 값을 받겠다
	...
	public String dayHealth(
			...
			@PathVariable("sc_id") String sc_id,
			Model model) {
		WorkOutDTO healthList = selfitService.getDayHealth(sc_id, listid);
		model.addAttribute("H_LIST", healthList);
